...
interface RecordData {
  ...
  DamageSheets?: number | string;
  LooseSheets?: number | string;
  UMCSheets?: number | string;
  MainPacket?: number | string;
}
...
const renderDay = useCallback(
  ...
  const handleDownloadPDF = () => {
    ...
    const doc = new jsPDF();
    doc.setFontSize(18);
    doc.text("Exam Sheet Summary", 14, 22);
    doc.setFontSize(12);
    ...
    doc.save("exam_sheet_summary.pdf");
  };
  ...
  const handleDownloadExcel = async () => {
    ...
    const worksheet = workbook.addWorksheet("Exam Sheet Summary");

    worksheet.columns = [
      { header: "Date", key: "Date", width: 15 },
      { header: "Exam Timing", key: "ExamTiming", width: 20 },
      { header: "Exam Center", key: "ExamCenter", width: 20 },
      { header: "Sheets", key: "Sheets", width: 15 },
      { header: "Present", key: "Present", width: 15 },
      { header: "UMC", key: "UMC", width: 15 },
      { header: "Consumed", key: "Consumed", width: 20 },
      { header: "Damage Sheets", key: "DamageSheets", width: 20 },
      { header: "Loose Sheets", key: "LooseSheets", width: 20 },
      { header: "UMC Sheets", key: "UMCSheets", width: 20 },
      { header: "Main Packet", key: "MainPacket", width: 20 },
    ];
    ...
    records
      ...
      .forEach((row) => {
        worksheet.addRow({
          ...
          DamageSheets: row.DamageSheets ?? "-",
          LooseSheets: row.LooseSheets ?? "-",
          UMCSheets: row.UMCSheets ?? "-",
          MainPacket: row.MainPacket ?? "-",
        });
      });
    ...
    a.href = url;
    a.download = "exam_sheet_summary.xlsx";
    a.click();
      ...
      {loading ? (
        ...
        <Paper sx={{ p: 3, borderRadius: "16px", boxShadow: 3 }}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography variant="h6">Sheet Summary</Typography>
            ...
            <Table stickyHeader>
              <TableHead>
                <TableRow>
                  ...
                  <TableCell sx={{ minWidth: 100 }}>Exam Center</TableCell>
                  <TableCell sx={{ minWidth: 100 }}>Sheets</TableCell>
                  <TableCell sx={{ minWidth: 80 }}>Present</TableCell>
                  <TableCell sx={{ minWidth: 150 }}>UMC</TableCell>
                  <TableCell sx={{ minWidth: 130 }}>Consumed</TableCell>
                  <TableCell sx={{ minWidth: 120 }}>Damage Sheets</TableCell>
                  <TableCell sx={{ minWidth: 150 }}>Loose Sheets</TableCell>
                  <TableCell sx={{ minWidth: 150 }}>UMC Sheets</TableCell>
                  <TableCell sx={{ minWidth: 170 }}>Main Packet</TableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {records
                  ...
                  .map((row, idx) => (
                    <TableRow key={idx}>
                      <TableCell>{`${row.Date}-${row.Month}-${row.Year}`}</TableCell>
                      <TableCell>{row.ExamTiming ?? "-"}</TableCell>
                      <TableCell>{row.ExamCenter ?? "-"}</TableCell>
                      <TableCell>{row.Sheets ?? "-"}</TableCell>
                      <TableCell>{row.Present ?? "-"}</TableCell>
                      <TableCell>{row.UMC ?? "-"}</TableCell>
                      <TableCell>{row.Consumed ?? "-"}</TableCell>
                      <TableCell>{row.DamageSheets ?? "-"}</TableCell>
                      <TableCell>{row.LooseSheets ?? "-"}</TableCell>
                      <TableCell>{row.UMCSheets ?? "-"}</TableCell>
                      <TableCell>{row.MainPacket ?? "-"}</TableCell>
                    </TableRow>
                  ))}
              </TableBody>
            </Table>
...
export default ExamSheetSummary;